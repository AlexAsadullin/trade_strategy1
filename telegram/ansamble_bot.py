...
load_dotenv()
TOKEN = '7451662695:AAHx0_ceDY_JAGQR4y-KKaG-4pKergFFd4E' #os.getenv('bitscalperasad_token')
...
def get_figi(name: str, instrument: str):
    ...
    return result.values[0] if len(result) > 0 else 'Ничего не найдено!'
...
def find_figi(message):
    ...
    print(figi[1])
    bot.send_message(message.chat.id, f'{figi}. Если подходит, нажми на /period', reply_markup=markup)
...
def stocks(message):
    ...
    bot.register_next_step_handler(msg, find_stock_period)

def find_stock_period(message):
    try:
        global USER_DATA
...
def stocks(message):
    ...
    fig.update_layout(title="График цены акций (Close)",
