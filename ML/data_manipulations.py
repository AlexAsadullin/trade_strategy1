...
def split_data(df: pd.DataFrame, train_part: float, scaler=None):
    ...
    if scaler:
        # Преобразуем X как есть (у него уже 2D)
        X_train_scaled = scaler.fit_transform(X_train.to_numpy(dtype=np.float32))
        X_test_scaled = scaler.transform(X_test.to_numpy(dtype=np.float32))

        # Преобразуем y в 2D, масштабируем, затем возвращаем обратно в 1D
        y_train_reshaped = y_train.to_numpy(dtype=np.float32).reshape(-1, 1)
        y_test_reshaped = y_test.to_numpy(dtype=np.float32).reshape(-1, 1)
...
def calculate_next_ratio(df, n):
    df[f'next_ratio'] = df['Close'].shift(-1) / df['Close']
    return df
...
def prepare_data_ratio(df: pd.DataFrame, data_write_path:str = '', n_prev_ratio=7, n_next_ratio=1, window_size=30):
    try:
    ...
    df = calculate_next_ratio(df, n_next_ratio)
